...
func (da *dataAggregator) ProcessHyperBlock(outportBlock *outport.OutportBlock) (*data.HyperOutportBlock, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to convert meta outport block: %w", err)
	}
	...
	for _, notarizedHash := range outportBlock.NotarizedHeadersHashes {
		...
		shardBlock, convertErr := da.converter.HandleShardOutportBlock(outportBlockShard)
		if convertErr != nil {
			return nil, fmt.Errorf("failed to convert shard outport block: %v", convertErr)
		}
